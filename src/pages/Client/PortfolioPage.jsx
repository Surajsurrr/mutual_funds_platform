              ...
              <motion.div key={h.schemeId} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i*0.08 }}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color: '#7a94ab' }}>{h.amcName}</p>
                    <div className="flex gap-4 mt-2 text-xs" style={{ color: '#7a94ab' }}>
                      <span>{h.units.toFixed(3)} units</span>
                      <span>Avg: ₹{h.avgNAV}</span>
                      <span>Cur: ₹{h.currentNAV}</span>
                    </div>
                    <div className="mt-3 h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(27,154,245,0.1)' }}>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-xs" style={{ color: '#7a94ab' }}>Invested: {formatCurrency(h.invested)}</p>
                    <p className={`text-sm font-bold mt-1 ${h.gain >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
